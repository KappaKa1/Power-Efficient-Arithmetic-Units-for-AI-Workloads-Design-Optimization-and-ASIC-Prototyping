...
module SRAM_controller #(
  ...
  output logic						reset_intermediate_regs_o,
  ...
  typedef enum logic [2:0] {IDLE, LATENCY_FROM_REG, WRITE_SRAM, GET_DATA_GEMM, COMPUTE_DONE, READ_SRAM, FINAL_DATA} sram_ctrl_state; // ENABLE/DISABLE CHECK_INPUT
  ...
  logic reset_intermediate_regs_q, reset_intermediate_regs_d;
  ...
  assign reset_intermediate_regs_o = reset_intermediate_regs_q;
     ...
     LATENCY_FROM_REG: begin
      ...
      WRITE_SRAM: begin 
        ...
        if(shift_reg_cnt_q == SHIFT_REG_CYCLES - 1) begin
          ...
          //sram_req_d[2] = '1; // DELETE
          //sram_we_d[2] = '1; // DELETE
          
	  if(input_sram_cnt_q == INPUT_SRAM_ACCESS_CYCLES - 1) begin 
	    input_sram_cnt_d = '0;
	    //output_sram_cnt_d = '0; // DELETE
	    start_d = '1;
	    state_d = GET_DATA_GEMM;

	  end else begin
	    input_sram_cnt_d = input_sram_cnt_q + 1;
	    //output_sram_cnt_d = output_sram_cnt_q + 1; // DELETE
	  end
       end else begin
  ...
  always_ff @(posedge clk_i) begin
    if (!rst_ni) begin
      ...
      mux_sel_q <= mux_sel_d;
      start_q <= start_d;
      reset_intermediate_regs_q <= reset_intermediate_regs_d;
